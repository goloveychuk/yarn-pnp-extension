...
type IterableType<T> = { [Symbol.iterator](): IterableIterator<T> };
...
function scoreForArr(query: string, items: IterableType<Locator>) {
...
function limitAndArray<T>(lim: number, gen: IterableType<T>) {
  const res: T[] = [];
...
function* getSearchRes(registry: Registry, query: string): Generator<Result> {
  const scores = limitAndArray(
    TOTAL_LIMIT,
    scoreForArr(query, registry.data.allLocators),
  );
...
function runJsondCommand<T>(cwd: string, cmd: string, args: string[]) {
  return iterMap(
    splitByNewline(runProcess(cwd, cmd, args)),
...
async function getPnpData(projectRoot: string) {
  ...
  for await (const item of pnpData) {
    ...
    const locatorStr = castStrToLocatorHash(
...
async function getProjectData(projectRoot: string) {
  ...
  for await (const item of data) {
    // const itemLoc = structUtils.parseLocator(
    //   castLocatorToStr(item.value),
    //   true,
    // );

    allLocators.push(item.value);
  ...
  return { reverseMap, depMap, allLocators };
}
